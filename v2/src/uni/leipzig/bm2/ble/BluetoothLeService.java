    ...
    private void broadcastUpdate(final String action,
        ...
        if (UUID_BOTTLE_HELLO.equals(characteristic.getUuid())) {
        	...
        	if ( bmData != null && bmData.length > 0 ) {
        		final StringBuilder bmStringBuilder = new StringBuilder(bmData.length);
        		for (byte byteChar : bmData)
        			bmStringBuilder.append(String.format("%02X ", byteChar));
        		Log.e(TAG, bmStringBuilder.toString());
        		intent.putExtra(EXTRA_DATA, new String(bmData) + "\n" + bmStringBuilder.toString() + "!");
        	}
...
//        	mBluetoothGatt.writeDescriptor(bmDescriptor);
        	Log.e(TAG, "Would set the notification for the descriptor.");
        }
    }
